...
// _VelocityTex.w   = 0
//
Shader "Hidden/Aaron/PointCloud/Flock"
{
    Properties
    {
        _PositionTex ("-", 2D) = ""{}
    ...
    float4 frag_update_velocity(v2f_img i) : SV_Target 
    {
		...
		fToNeighbors = (pCenter + float3(0.5-uv.y, abs(0.5-uv.y)*sin((1-nrand(float2(uv.x, 0), 6)/10)*_TimeParams.x),0)) - p;
