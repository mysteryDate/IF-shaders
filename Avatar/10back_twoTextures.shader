Shader "Aaron/Avatar/10_backTwoTextures" {
	Properties {
		_BackTex ("Back Texture", 2D) = "BlueSpace" {}
		_BumpTex ("Bumpmap", 2D) = "bump" {}
		_BumpDepth ("BumpDepth", Range(0.01,3.0)) = 2.2
		[HideInInspector] _TextureShift ("Texture Shift", Vector) = (0.54,0.7,0,0)
		[HideInInspector] _TextureScale ("Texture Scale", Vector) = (8.55,27.53,0,0)
		[HideInInspector] _IFPos ("IF Position", Vector) = (0,0,0,0)
		[HideInInspector] _WaterHeight ("Water Height", float) = 0
		_WaterTint ("Water Tint", Color) = (1.0,1.0,1.0,1.0)
		[HideInInspector] 
		_NewTex ("New Texture", 2D) = "BlueSpace" {}
		[HideInInspector] 
		_TextureBlend ("Texture Blend", Range(0,1)) = 1
		[MaterialToggle] _Occlude ("Front of IF occlusion?", float) = 0
	}
	SubShader {
		...
		Pass {
			...
			float4 frag(vertexOutput i) : COLOR 
			{
				...
				if( length(i.posWorld.xyz - i.cameraPos) + 1.5 < length(_IFPos.xyz - i.cameraPos ) && _Occlude) {
					discard;
				}
		...
		Pass {
			...
			float4 frag(vertexOutput i) : COLOR 
			{
				...
				if( length(i.posWorld.xyz - cameraPos.xyz) + 1.5 < length(_IFPos.xyz - cameraPos.xyz ) && _Occlude) {
					alpha = dist/2;
				}

				float4 tex = tex2D(_BackTex, i.tex.xy);
				float4 newTex = tex2D(_NewTex, i.tex.xy);
				
				tex = _TextureBlend * tex + (1 - _TextureBlend) * newTex;
